...
/*
void forward_ntt(uint16_t r[SABER_N]) {
    unsigned int len, start, j, k;
    int16_t t, zeta;

    k = 1;
    for (len = SABER_N / 2; len >= 2; len >>= 1) {
        for (start = 0; start < SABER_N; start = j + len) {
            zeta = zetas[k++];  // Zeta değerleri SABER'e uygun olarak ayarlanmalı
            for (j = start; j < start + len; j++) {
                t = fqmul(zeta, r[j + len]);  // SABER'in modül işlemleri burada yapılmalı
                r[j + len] = r[j] - t;
                r[j] = r[j] + t;
            }
        }
